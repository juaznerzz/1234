...
app.post('/validate', async (req, res) => {
  ...
  // Verificar que sub_key y unique_id existan
  if (!sub_key || !unique_id) {
    return res.status(400).json({ valid: false, message: 'Faltan sub_key o unique_id' });
  }

  try {
    // Buscar licencia en Supabase por sub_key y unique_id (user_id = unique_id)
    const params = new URLSearchParams();
    params.append('sub_key', `eq.${sub_key}`);
    params.append('user_id', `eq.${unique_id}`);
    ...
    // Responder con licencia válida y datos necesarios
    return res.json({
      valid: true,
